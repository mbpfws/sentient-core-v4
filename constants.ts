...
The response must be in ${lang === 'vi' ? 'Vietnamese' : 'English'}.`,
    ...
    },
    {
      ...
      synthesisPrompt: (projectDesc, prev, lang) => `Synthesize the following documents into a coherent brief for an architect in JSON format with keys for business_goals, tech_constraints, and summary. The goal is to inform the System Architecture design. Extract key business goals from the Executive Summary and technical constraints/choices from the Tech Stack. Project: "${projectDesc}"\n\nExecutive Summary:\n${prev['n2']}\n\nTech Stack:\n${prev['n3']}\n\nThe response must be in ${lang === 'vi' ? 'Vietnamese' : 'English'}.`,
    },
    {
      id: 'n4',
    ...
    },
    {
      ...
      outlinePrompt: (projectDesc, synthesis, lang) => `You are a Database Administrator. Based on the system architecture ("${synthesis}") for project "${projectDesc}", create an outline of the core database tables required. For each table, list the key columns you plan to include. Output in JSON format. This is a high-level plan, not the final SQL. The response must be in ${lang === 'vi' ? 'Vietnamese' : 'English'}.`,
      taskPrompt: (_pd, synthesis, outline, lang) => `Based on the architectural context ("${synthesis}") and the approved schema outline below, write the complete and normalized SQL DDL (CREATE TABLE statements) for the project in markdown with metadata tags. Ensure you include primary keys, foreign key constraints, indexes, and appropriate data types (e.g., UUID, TEXT, TIMESTAMPTZ).\n\nApproved Outline:\n${outline}\n\nThe response must be in ${lang === 'vi' ? 'Vietnamese' : 'English'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 'n6',
      ...
      outlinePrompt: (projectDesc, synthesis, lang) => `You are a Backend Engineer. Based on the system architecture ("${synthesis}") for project "${projectDesc}", create an outline of the key API resources and their primary endpoints (e.g., /users, GET, POST). For each endpoint, briefly describe its purpose. Output in JSON format. The response must be in ${lang === 'vi' ? 'Vietnamese' : 'English'}.`,
      taskPrompt: (_pd, synthesis, outline, lang) => `Using the architectural context ("${synthesis}") and the approved API outline below, generate a partial OpenAPI 3.0.x specification in YAML format with metadata comments. Detail the paths, operations (GET, POST), parameters, and basic request/response schemas for the 2-3 most critical endpoints.\n\nApproved Outline:\n${outline}\n\nThe response must be in ${lang === 'vi' ? 'Vietnamese' : 'English'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 'n7',
  ...
  nodes: [
    {
      ...
      taskPrompt: (projectDesc, _s, _o, lang) => `Phân tích mô tả dự án sau và xuất kết quả dưới dạng JSON với các khóa: "core_problem", "target_users", "key_features", "complexity_level", "summary". Xác định vấn đề cốt lõi, đối tượng người dùng chính và các tính năng chính. Xác định điểm phức tạp (Thấp, Trung bình, Cao). Cung cấp một bản tóm tắt ngắn gọn. Dự án: "${projectDesc}". Phản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 'n2',
      ...
      outlinePrompt: (projectDesc, synthesis, lang) => `Bạn là một Quản lý Dự án Cấp cao. Tạo một đề cương chi tiết cho một bản Tóm tắt Điều hành dài một trang cho một dự án phần mềm và xuất dưới dạng JSON với cấu trúc: {"sections": [{"title": "...", "description": "...", "key_points": ["..."]}]}. Dự án là: "${projectDesc}". Phân tích ban đầu là: "${synthesis}". Đề cương nên có các phần rõ ràng cho Tầm nhìn Dự án, Tuyên bố Vấn đề, Giải pháp Đề xuất, Mục tiêu Chính (mục tiêu SMART), Phạm vi (Trong phạm vi/Ngoài phạm vi) và một Lộ trình Chiến lược cấp cao (MVP, Giai đoạn 2). Phản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      taskPrompt: (_pd, synthesis, outline, lang) => `Sử dụng đề cương và phân tích ban đầu sau đây, viết một bản Tóm tắt Điều hành chuyên nghiệp, toàn diện dưới dạng Markdown với các thẻ metadata ở đầu (---\ntitle: "..."\ntype: "executive_summary"\ntags: ["..."]\n---). Bản tóm tắt phải chi tiết, thuyết phục và phù hợp với các bên liên quan của dự án. Phân tích ban đầu: "${synthesis}".\n\nĐề cương đã được phê duyệt:\n${outline}\n\nPhản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 'n3',
      ...
      outlinePrompt: (projectDesc, synthesis, lang) => `Bạn là một Giám đốc Công nghệ. Tạo một đề cương cho tài liệu Đề xuất Bộ Công nghệ và xuất dưới dạng JSON với cấu trúc: {"technology_categories": [{"category": "...", "description": "...", "considerations": ["..."]}]}. Dự án: "${projectDesc}". Phân tích ban đầu là: "${synthesis}". Đề cương nên bao gồm: Framework Frontend, Framework/Ngôn ngữ Backend, Cơ sở dữ liệu (SQL/NoSQL), Dịch vụ Xác thực, Nền tảng Triển khai/Lưu trữ và Tích hợp AI/ML (nếu có). Sử dụng Google Search để tìm các công nghệ hiện đại, có khả năng mở rộng phù hợp cho việc ra mắt vào năm 2025. Phản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      taskPrompt: (_pd, synthesis, outline, lang) => `Dựa trên phân tích dự án ("${synthesis}") và đề cương đã được phê duyệt dưới đây, tạo một tài liệu Đề xuất Bộ Công nghệ chi tiết dưới dạng Markdown với metadata (---\ntitle: "..."\ntype: "technology_stack"\ntags: ["..."]\n---). Đối với mỗi mục trong đề cương, cung cấp một lựa chọn công nghệ cụ thể (ví dụ: Next.js, FastAPI, PostgreSQL, NextAuth.js, Vercel) và viết một lý do mạnh mẽ, có dữ liệu chứng minh tại sao nó là lựa chọn tốt nhất cho dự án này, xem xét khả năng mở rộng, hiệu suất và hệ sinh thái nhà phát triển. Sử dụng thông tin mới nhất từ kết quả tìm kiếm của bạn.\n\nĐề cương đã được phê duyệt:\n${outline}\n\nPhản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 's1',
      ...
      synthesisPrompt: (projectDesc, prev, lang) => `Tổng hợp các tài liệu sau thành một bản tóm tắt mạch lạc cho một kiến trúc sư và xuất dưới dạng JSON với các khóa: "business_objectives", "technical_constraints", "technology_choices", "architectural_requirements", "synthesis_summary". Mục tiêu là để thông báo cho thiết kế Kiến trúc Hệ thống. Trích xuất các mục tiêu kinh doanh chính từ Tóm tắt Điều hành và các ràng buộc/lựa chọn kỹ thuật từ Bộ Công nghệ. Dự án: "${projectDesc}"\n\nTóm tắt Điều hành:\n${prev['n2']}\n\nBộ Công nghệ:\n${prev['n3']}\n\nPhản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
    },
    {
      id: 'n4',
      ...
      outlinePrompt: (projectDesc, synthesis, lang) => `Bạn là một Kiến trúc sư Giải pháp chính. Dựa trên bản tóm tắt thiết kế này: "${synthesis}", tạo một đề cương cho tài liệu Kiến trúc Hệ thống và xuất dưới dạng JSON với cấu trúc: {"architecture_sections": [{"section": "...", "description": "...", "components": ["..."]}]}. Dự án: "${projectDesc}". Đề cương phải bao gồm các phần cho: Các thành phần cốt lõi (ví dụ: Ứng dụng Frontend, Cổng API, Dịch vụ), Luồng dữ liệu, Chiến lược xác thực và kế hoạch cho một sơ đồ MermaidJS để trực quan hóa các thành phần. Phản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      taskPrompt: (_pd, synthesis, outline, lang) => `Sử dụng bản tóm tắt thiết kế ("${synthesis}") và đề cương đã được phê duyệt, viết một tài liệu Kiến trúc Hệ thống chi tiết dưới dạng Markdown với metadata (---\ntitle: "..."\ntype: "system_architecture"\ntags: ["..."]\n---). Mô tả trách nhiệm của từng thành phần và sự tương tác của chúng. Sau phần mô tả, cung cấp một khối mã sơ đồ đồ thị MermaidJS hợp lệ (sử dụng \`\`\`mermaid) đại diện trực quan cho kiến trúc được mô tả.\n\nĐề cương đã được phê duyệt:\n${outline}\n\nPhản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 'n5',
      ...
      outlinePrompt: (projectDesc, synthesis, lang) => `Bạn là một Quản trị viên Cơ sở dữ liệu. Dựa trên kiến trúc hệ thống ("${synthesis}") cho dự án "${projectDesc}", tạo một đề cương về các bảng cơ sở dữ liệu cốt lõi cần thiết và xuất dưới dạng JSON với cấu trúc: {"database_tables": [{"table_name": "...", "purpose": "...", "key_columns": ["..."]}]}. Đối với mỗi bảng, liệt kê các cột chính bạn dự định bao gồm. Đây là một kế hoạch cấp cao, không phải là SQL cuối cùng. Phản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      taskPrompt: (_pd, synthesis, outline, lang) => `Dựa trên bối cảnh kiến trúc ("${synthesis}") và đề cương lược đồ đã được phê duyệt dưới đây, viết SQL DDL (câu lệnh CREATE TABLE) hoàn chỉnh và được chuẩn hóa cho dự án dưới dạng Markdown với metadata (---\ntitle: "..."\ntype: "database_schema"\ntags: ["..."]\n---). Đảm bảo bạn bao gồm các khóa chính, ràng buộc khóa ngoại, chỉ mục và các kiểu dữ liệu phù hợp (ví dụ: UUID, TEXT, TIMESTAMPTZ).\n\nĐề cương đã được phê duyệt:\n${outline}\n\nPhản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 'n6',
      ...
      outlinePrompt: (projectDesc, synthesis, lang) => `Bạn là một Kỹ sư Backend. Dựa trên kiến trúc hệ thống ("${synthesis}") cho dự án "${projectDesc}", tạo một đề cương về các tài nguyên API chính và các điểm cuối chính của chúng và xuất dưới dạng JSON với cấu trúc: {"api_resources": [{"resource": "...", "endpoints": [{"method": "...", "path": "...", "purpose": "..."}]}]}. Đối với mỗi điểm cuối, mô tả ngắn gọn mục đích của nó. Phản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      taskPrompt: (_pd, synthesis, outline, lang) => `Sử dụng bối cảnh kiến trúc ("${synthesis}") và đề cương API đã được phê duyệt dưới đây, tạo một đặc tả OpenAPI 3.0.x một phần ở định dạng YAML dưới dạng Markdown với metadata (---\ntitle: "..."\ntype: "api_design"\ntags: ["..."]\n---). Chi tiết hóa các đường dẫn, hoạt động (GET, POST), tham số và các lược đồ yêu cầu/phản hồi cơ bản cho 2-3 điểm cuối quan trọng nhất.\n\nĐề cương đã được phê duyệt:\n${outline}\n\nPhản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 'n7',
      ...
      outlinePrompt: (projectDesc, synthesis, lang) => `Bạn là một Nhà thiết kế UI/UX. Dựa trên kiến trúc hệ thống ("${synthesis}") cho dự án "${projectDesc}", tạo một đề cương về các giao diện người dùng và trang chính cần được thiết kế và xuất dưới dạng JSON với cấu trúc: {"ui_interfaces": [{"interface": "...", "purpose": "...", "components": ["..."], "interactions": ["..."]}]}. Đối với mỗi giao diện, mô tả mục đích, các thành phần chính (nút, biểu mẫu, điều hướng) và tương tác người dùng. Bao gồm đặc tả wireframe cho cấu trúc bố cục. Phản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      taskPrompt: (_pd, synthesis, outline, lang) => `Sử dụng bối cảnh kiến trúc ("${synthesis}") và đề cương UI đã được phê duyệt dưới đây, tạo các mockup chuyên nghiệp dựa trên SVG cho các giao diện người dùng chính dưới dạng Markdown với metadata (---\ntitle: "..."\ntype: "ui_mockups"\ntags: ["..."]\n---). Tạo các thiết kế sạch sẽ, hiện đại với kiểu chữ, khoảng cách và thứ bậc trực quan phù hợp. Đối với mỗi giao diện, cung cấp:\n\n1. Mã SVG hoàn chỉnh với cấu trúc phù hợp\n2. Cân nhắc thiết kế responsive\n3. Đặc tả thành phần (nút, biểu mẫu, điều hướng)\n4. Tính năng truy cập (nhãn ARIA, độ tương phản phù hợp)\n5. Trạng thái phần tử tương tác (hover, focus, disabled)\n\nTập trung vào việc tạo ra các giao diện thực tế, chuyên nghiệp có thể được sử dụng trực tiếp cho phát triển.\n\nĐề cương đã được phê duyệt:\n${outline}\n\nPhản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 'n8',
      ...
      outlinePrompt: (projectDesc, synthesis, lang) => `Bạn là một Kiến trúc sư Giải pháp. Dựa trên thiết kế hệ thống ("${synthesis}") cho dự án "${projectDesc}", tạo một đề cương cho các sơ đồ kiến trúc toàn diện và xuất dưới dạng JSON với cấu trúc: {"diagram_types": [{"type": "...", "purpose": "...", "elements": ["..."], "relationships": ["..."]}]}. Bao gồm kế hoạch cho: Tổng quan Kiến trúc Hệ thống, Sơ đồ Luồng Dữ liệu, Thiết kế Lớp API, Mối quan hệ Cơ sở dữ liệu (ERD), Pipeline Triển khai và Phụ thuộc Thành phần. Đối với mỗi loại sơ đồ, chỉ định các phần tử và mối quan hệ nào cần được trực quan hóa. Phản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      taskPrompt: (_pd, synthesis, outline, lang) => `Sử dụng bối cảnh kiến trúc ("${synthesis}") và đề cương sơ đồ đã được phê duyệt dưới đây, tạo các sơ đồ Mermaid toàn diện cho kiến trúc hệ thống dưới dạng Markdown với metadata (---\ntitle: "..."\ntype: "architecture_diagrams"\ntags: ["..."]\n---). Tạo các loại sơ đồ sau:\n\n1. **Kiến trúc Hệ thống**: Tổng quan thành phần cấp cao sử dụng sơ đồ kiến trúc Mermaid\n2. **Luồng Dữ liệu**: Luồng quy trình hiển thị chuyển động dữ liệu giữa các thành phần\n3. **Thiết kế API**: Sơ đồ tuần tự cho các tương tác API chính\n4. **Lược đồ Cơ sở dữ liệu**: Sơ đồ Mối quan hệ Thực thể hiển thị mối quan hệ bảng\n5. **Pipeline Triển khai**: Sơ đồ luồng hiển thị các giai đoạn CI/CD\n6. **Phụ thuộc Thành phần**: Đồ thị hiển thị phụ thuộc module và dịch vụ\n\nĐối với mỗi sơ đồ, cung cấp:\n- Cú pháp Mermaid hợp lệ trong khối mã\n- Nhãn và mô tả rõ ràng\n- Mối quan hệ và kết nối phù hợp\n- Kiểu dáng và bố cục chuyên nghiệp\n\nĐề cương đã được phê duyệt:\n${outline}\n\nPhản hồi phải bằng ${lang === 'vi' ? 'tiếng Việt' : 'tiếng Anh'}.`,
      refinePrompt: defaultRefinePrompt,
    },
    {
      id: 'n9',
